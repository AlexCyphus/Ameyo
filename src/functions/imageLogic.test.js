import {handleChangeBackground, toggleAllowedBackgroundChanges} from './imageLogic.js'
...
const thisHolder = {
    background: toggleAllowedBackgroundChanges,
    setState: (newState) => {
        for (var key of Object.keys(newState)) {
            thisHolder.state[key] = newState[key]
        }
    },
    state: {}
}
...
beforeEach(() => {
    localStorage.backgroundImageIndex = "3"
    document.body.style.backgroundImage = 'url(oldImage)'   
});

describe('handleChangeBackground', () => {
    it('Changes background when not prevented', () => {
        localStorage['preventChangeBackground'] = false
    
        handleChangeBackground()
    
        expect(localStorage.backgroundImageIndex).toBe("4")
        expect(document.body.style.backgroundImage).toBe(imageUrls[localStorage.backgroundImageIndex])
    });
    
    it ("Doesn't change the background when it shouldn't", () => {
        localStorage['preventChangeBackground'] = true
    
        handleChangeBackground()
        
        expect(localStorage.backgroundImageIndex).toBe("3")
        expect(document.body.style.backgroundImage).toBe('url(oldImage)')
    })

    it ("Changes it if skipBackground is true (PREVIOUS)", () => {
        localStorage['preventChangeBackground'] = true

        handleChangeBackground('last')

        expect(localStorage.backgroundImageIndex).toBe("2")
        expect(document.body.style.backgroundImage).toBe(imageUrls[localStorage.backgroundImageIndex])
    })

    it ("Changes it if skipBackground is true (NEXT)", () => {
        localStorage['preventChangeBackground'] = true

        handleChangeBackground('next')
...
describe('toggleAllowedBackgroundChanges', () => {
    it('Toggles true to false', () => {
        localStorage.preventChangeBackground = true 

        thisHolder.background()
        expect(thisHolder.state['preventChangeBackground']).toBe(false)
        expect(localStorage.preventChangeBackground).toBe('false')
    })
    ...
    it('Toggles false to true', () => {
        ...
        thisHolder.background()
        expect(thisHolder.state['preventChangeBackground']).toBe(true)
        expect(localStorage.preventChangeBackground).toBe('true')
    })
})